...
function sendFilmsByQuery(chatId, query) {
  Film.find(query).then((films) => {
    ...
    const uniqueFilms = films.filter(
      (film, index, self) =>
        index ===
        self.findIndex((f) => f.name === film.name && f.uuid === film.uuid)
    );

    const html = uniqueFilms
      .map((f, i) => {
...
function showFavouriteFilms (chatId, telegramId) {
  User.findOne({telegramId})
    .then(user => {
      if (user) {
        const uniqueFilmUuids = [...new Set(user.films)];

        Film.find({uuid: {'$in': uniqueFilmUuids}}).then(films => {
          const uniqueFilms = films.filter((film, index, self) =>
            index === self.findIndex((f) => f.uuid === film.uuid)
          );

          let html

          if (uniqueFilms.length) {
            html = uniqueFilms.map((f, i) => {
